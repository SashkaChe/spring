...
public class CountryDAO {
	...
	public List<Country> showCountry() {
		return sessionFactory.getCurrentSession().createQuery("select distinct c from Country c left join fetch c.res")
				.list();
	}
	...
	public Country showCountryId(long id) {
		return (Country) sessionFactory.getCurrentSession()
				.createQuery("select c from Country c left join fetch c.res where c.id = :id").setParameter("id", id)
				.uniqueResult();
	}
	...
	public City showCityId(int id) {
		return (City) sessionFactory.getCurrentSession().createQuery("select c from City c where c.id = :id")
